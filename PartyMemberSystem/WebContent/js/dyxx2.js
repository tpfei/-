...
//			url: "../json/dyxx.json",
	...
	function cxdy(data) {
		...
		$.each(data, function (i, partMember) {
			...
			var td9 = $('<td>');
			td9.append($('<input>').attr('type', 'button').attr('value', '删除').attr('class','delete').attr('name',partMember['dyId']));
			td9.append($('<input>').attr('type', 'button').attr('value', '编辑').attr('class','edit').attr('name',partMember['name']));
			tr.append(td1);
			tr.append(td2);
			tr.append(td3);
			tr.append(td4);
		...
		$(".edit").click(function() {
			...
			window.open('edit.html?'+name,'select','height=600px,width=900px,top='+(screen.availHeight-600)/2+',left='+(screen.availWidth-900)/2+',status=yes');
